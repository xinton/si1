	...
	public String geraNumero() throws Exception {
		...
		if(input.equals("")){
			...
			if(input.length() == 4 && input.startsWith("1")){
				...
				String centenas = input.substring(input.length()-3, input.length());
				result += geraNumero(input.substring(0, (input.length()-3))) + " mil";
				if(!centenas.equals("000") && centenas.startsWith("0")){
					result += " e";
				}else if(centenas.equals("000")){
					return result;
				}
				result += " " + geraNumero(centenas);
			}else if(input.length() >= 7 && input.length() <= 9){
				String centenas = input.substring(input.length()-3, input.length());
				String milhares = input.substring(input.length() - 6, input.length() - 3);
				String milhões = input.substring(0, input.length()-6);
				if(input.length() == 7 && input.startsWith("1")){
					result += "um milhão";
				}else{
					result += geraNumero(milhões) + " milhões";
					if(centenas.equals("000") && milhares.equals("000")){
						return result;
					}else if(centenas.equals("000") && !milhares.equals("000")){
						result += " " + geraNumero(milhares) + " mil";
					}else if(!centenas.equals("000") && milhares.equals("000")){
						result += " e " + geraNumero(centenas);
					}else{
						result += " " + geraNumero(milhares) + " mil " + geraNumero(centenas); 
					}
				}
			}else{
